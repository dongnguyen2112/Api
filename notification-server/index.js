...
app.use(cors({
    origin: ' http://192.168.2.8:8081', // Thêm địa chỉ của app frontend
...
// Hàm gửi thông báo khi tiếp nhận sự cố
app.post('/send-incident-notification', async (req, res) => {
    console.log('Request body:', req.body); // In ra toàn bộ body request
  
    const { expoPushToken, title, body } = req.body;
    const message = {
      to: expoPushToken,
      sound: 'default',
      title: title || 'Thông báo tiếp nhận',  // Sử dụng title mặc định nếu không có
      body: body || 'Vấn đề của bạn đã được tiếp nhận.', // Sử dụng body mặc định nếu không có
      data: { type: "incident" },
    };
    ...
    try {
      ...
      res.status(500).json({ error: 'Có lỗi xảy ra khi gửi thông báo sự cố.' });
    }
  });
